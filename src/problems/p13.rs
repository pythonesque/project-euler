...
static digits: uint = 50;

static numbers: [u8, ..5_000] = [
...
fn add(digit: uint, carry: u16) -> (u8, u16) {
    let sum = carry + if digit < digits {
        range_step(digits - digit - 1, digits * (rows + 1) - digit - 1, digits)
        .map(|index| *(numbers.get(index).unwrap()) as u16)
        .sum()
    } else { 0 };
...
euler_problem!(b"361113f19fd302adc31268f8283a4f2d", w, {
    ...
    let mut carry = 0;
    let mut digit = 0;
    while digit < digits || carry > 0 {
        let (remainder, new_carry) = add(digit, carry);
        answer[digit % remember_digits] = remainder;
        carry = new_carry;
        digit += 1
    }
